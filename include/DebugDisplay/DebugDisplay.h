...
public:
  ...
  /**
   * @brief Converts a 2D vector of ValueType (e.g. double) to a grayscale cv::Mat internally and displays it
   * @param InputVector: grayscale representation of an image
   * @param NormalizationFactor: Factor by which to normalize the values such that any value is no larger than 1.0
   */
  template<typename ValueType>
  bool Display2DVector(const vector<vector<ValueType>>& InputVector, ValueType NormalizationFactor);
   ...
   * @param 
   */
  bool MarkPixelsAndDisplay(const vector<vector<bool>>& PixelsToMark, const cv::Mat& ImageToMark, uchar Color = 255);
...
template<typename ValueType>
bool KDebugDisplay::Display2DVector(const vector<vector<ValueType>>& InputVector, ValueType NormalizationFactor)
{
...
bool KDebugDisplay::MarkPixelsAndDisplay(const vector<vector<bool>>& PixelsToMark, const cv::Mat& ImageToMark, uchar Color)
{
